...
export default function DayDetailModal({ date, tasks, onClose, onEditTask, onAddTask }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/40 animate-in fade-in duration-200 manrope-body">
      <div className="bg-background w-full max-w-[450px] border-[0.5px] border-border rounded-none shadow-2xl flex flex-col max-h-[80vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-sidebar/10">
          <div className="flex items-center gap-3">
            <Calendar className="w-4 h-4 text-accent" />
            <h3 className="inconsolata-ui text-[11px] font-bold uppercase tracking-[0.2em] text-text-heading">
          ...
          {tasks.length > 0 ? (
            tasks.map((task) => (
              <button
                ...
                className={`w-full text-left p-4 rounded-none border transition-all hover:brightness-95 active:scale-[0.99] flex flex-col gap-2 ${getTaskColor(task.task_type, task.is_completed)}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {getTaskIcon(task.task_type)}
                    <span className="text-[12px] font-bold tracking-tight">{task.title}</span>
                  </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-border bg-sidebar/5">
          <button 
            onClick={onAddTask}
            className="w-full flex items-center justify-center gap-2 py-3 bg-text-heading text-background text-[11px] font-bold uppercase tracking-widest hover:opacity-90 transition-all"
          >

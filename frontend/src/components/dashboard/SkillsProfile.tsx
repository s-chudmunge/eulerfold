            ...
            <div>
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-black text-text-heading uppercase tracking-tight">Technical Proficiency</h2>
                    </div>
                    <button 
                        onClick={() => setShowWeightsInfo(!showWeightsInfo)}
                        className="p-2 hover:bg-sidebar hover:bg-header rounded-lg transition-colors text-text-muted"
                ...
                {showWeightsInfo && (
                    <div className="mb-6 p-4 bg-accent/10 border border-accent/20 rounded-lg animate-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                            {[
                                { label: 'Homework (40%)', desc: "Projects You've Solved & Homework submissions." },
                                ...
                                <div key={i}>
                                    <p className="text-[9px] font-black text-accent">{w.label}</p>
                                    <p className="text-[8px] text-accent opacity-80 uppercase tracking-tighter leading-tight">{w.desc}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                            ...
                            >
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <h3 className="text-sm font-black text-text-heading leading-snug tracking-tight">
                                            {skill.name}
                                        </h3>
                                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shrink-0 ${skill.tier === 'strong' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-accent/10 text-accent'}`}>
                                            {skill.tier}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-3 text-[10px] font-bold text-text-muted">
                                        <span className="text-text-heading">{Math.round(skill.confidence_score)}</span>
                                        <div className="w-24 h-1 bg-sidebar rounded-full overflow-hidden">
                                            <div className="bg-teal-600 h-full rounded-full" style={{ width: `${skill.confidence_score}%` }}></div>
                                        </div>
                                    </div>
                                </div>
                                {expandedSkillId === skill.id ? <ChevronUp className="h-4 w-4 text-text-muted opacity-60" /> : <ChevronDown className="h-4 w-4 text-text-muted opacity-60" />}
                            </button>
                            ...
                            {expandedSkillId === skill.id && (
                                <div className="px-4 pb-4 animate-in slide-in-from-top-2 duration-200">
                                    <div className="pt-4 border-t border-border grid grid-cols-2 gap-6">
                                        <div>
                                            <p className="text-[9px] font-black text-text-muted uppercase tracking-widest mb-3">Signal Contributions</p>
                                            <div className="space-y-2">
                                                {[
                                                    { label: "Homework Submitted", val: (skill.pow_score * 40).toFixed(1), total: 40 },
                                                    ...
                                                    <div key={i} className="flex justify-between items-center text-[10px]">
                                                        ...
                                                        <span className="font-black text-text-heading">{Math.min(Number(s.val), s.total).toFixed(1)} / {s.total}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-text-muted uppercase tracking-widest mb-3">Metadata</p>
                ...
                <div className="bg-header rounded-lg border border-border overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black text-text-heading uppercase tracking-widest">Training History</h3>
                        </div>
                        {isTrainingExpanded ? <ChevronUp className="h-4 w-4 text-text-muted opacity-60" /> : <ChevronDown className="h-4 w-4 text-text-muted opacity-60" />}
                    </button>
                    {isTrainingExpanded && (
                        <div className="p-4 pt-0 text-xs text-text-muted leading-relaxed animate-in slide-in-from-top-2 duration-200">
                ...
                <div className="bg-header rounded-lg border border-border overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xs font-black text-text-heading uppercase tracking-widest">Verification System</h3>
                        </div>
                        {isSystemExpanded ? <ChevronUp className="h-4 w-4 text-text-muted opacity-60" /> : <ChevronDown className="h-4 w-4 text-text-muted opacity-60" />}
                    </button>
                    {isSystemExpanded && (
                        <div className="p-4 pt-0 animate-in slide-in-from-top-2 duration-200">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[10px] font-bold text-text-muted uppercase tracking-tight">
                                {[
                                    'Automated progress tracking',
                                    'AI-evaluated homework',

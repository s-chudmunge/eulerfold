  ...
  const renderAvatars = () => (
    <>
      {CREATORS.map((creator, idx) => (
        <div key={idx} className="flex items-center gap-3 bg-sidebar/50 border border-border/50 px-3 py-1.5 rounded-full hover:bg-sidebar transition-colors cursor-default">
          <img 
            src={`/creators/${creator.handle}.jpg`} 
            alt={creator.name} 
            className="w-6 h-6 rounded-full object-cover bg-background border border-border"
            loading="lazy"
            onError={(e) => { e.currentTarget.src = `https://ui-avatars.com/api/?name=${creator.name}&background=random`; }}
          />
          <span className="font-sans text-[13px] font-semibold tracking-tight whitespace-nowrap text-text-primary">
            {creator.name}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="w-full text-center overflow-hidden border-t border-border/30 pt-6 mt-6">
      ...
      <div className="relative flex overflow-hidden w-full max-w-[100vw] [mask-image:linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
        <div className="flex animate-infinite-scroll-reverse items-center gap-x-6 md:gap-x-12 w-max shrink-0 pr-6 md:pr-12 opacity-[0.8] hover:[animation-play-state:paused]">
          {renderAvatars()}
        </div>
        <div aria-hidden="true" className="flex animate-infinite-scroll-reverse items-center gap-x-6 md:gap-x-12 w-max shrink-0 pr-6 md:pr-12 opacity-[0.8] hover:[animation-play-state:paused]">
          {renderAvatars()}
        </div>
      </div>
    </div>

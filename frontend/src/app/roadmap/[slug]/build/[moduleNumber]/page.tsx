...
import { ChevronLeft, Rocket, Shield, Info, CheckCircle, ExternalLink, Github, FileText, Layout, Plus, Sparkles, User as UserIcon, Code, Palette, Search, AlertCircle, X, GraduationCap } from 'lucide-react';
...
export default function BuildPilotPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/roadmap/${slug}?success=true`);
    } catch (err: any) {
  ...
  if (error || !roadmap) return (
    <div className="min-h-screen bg-background flex items-center justify-center p-8 text-center">
      <div className="max-w-md">
        ...
        <Link href={`/roadmap/${slug}`} className="inline-flex items-center gap-2 px-6 py-3 bg-teal-700 text-white rounded-none inconsolata-ui text-[11px] font-bold tracking-widest uppercase hover:opacity-90 transition-all shadow-lg shadow-teal-700/20">
      ...
      <header className="h-9 border-b border-border flex items-center justify-between px-6 bg-background z-[60] shrink-0">
        ...
        <div className="flex items-center gap-3">
           ...
           <div className="flex items-center gap-1">
             ...
             <Link href={`/roadmap/${slug}`} className="p-1.5 hover:bg-sidebar transition-colors text-text-muted hover:text-text-heading" title="Return to Roadmap">

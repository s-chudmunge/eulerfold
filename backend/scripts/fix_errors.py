...
print(f"Found {len(to_fix)} failed/error topics to fix.")
...
for row in rows:
    ...
    try:
        ...
        with urllib.request.urlopen(vid_req) as vid_response:
            ...
            # If duration is still too short, let's just bypass the duration check for manual fixes
            # as long as it's at least 2 minutes to prevent actual shorts.
            if mins < 2:

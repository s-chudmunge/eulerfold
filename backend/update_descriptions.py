...
async def update_courses():
    logging.info("Starting roadmap description updates...")
    for course_id, new_desc in DESCRIPTIONS.items():
        try:
            res = supabase.table('roadmaps').update({'description': new_desc}).eq('id', course_id).execute()
            if res.data:
                logging.info(f"Successfully updated course {course_id}")
            else:
                logging.warning(f"No rows updated for course {course_id} - check if ID exists")
        except Exception as e:
            logging.error(f"Failed to update course {course_id}: {e}")
    logging.info("Completed updates.")

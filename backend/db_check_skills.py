...
def main():
    ...
    us_res = sb.table("user_skills").select("id, canonical_skill_id, contributing_roadmap_ids").eq("user_id", uid).execute()
    for us in us_res.data:
        c_res = sb.table("canonical_skills").select("name").eq("id", us["canonical_skill_id"]).execute()
        if c_res.data:
            print(f"Skill: {c_res.data[0]['name']}, Roadmaps: {us['contributing_roadmap_ids']}")

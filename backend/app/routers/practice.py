...
async def _generate_mcq_questions(
    ...
    Generate {num_questions} Multiple Choice Questions (MCQs) for a learner currently in Module/Week {week}{f' ("{module_title}")' if module_title else ''}.
    ...
    try:
        ...
        # Clean reasoning tokens or code blocks if present
        ...
        if isinstance(questions, dict):
            # Extract questions if wrapped inside a dictionary
            questions = questions.get("questions") or questions.get("data") or questions.get("mcqs") or list(questions.values())[0] if questions else []
            
        if not isinstance(questions, list):
            logger.error(f"MCQ generator did not return a list. Got: {type(questions)}")
            return [], usage
...
async def generate_mcq_session(data: MCQSessionCreate, current_user: User = Depends(get_current_user)):
    display_title = data.module_title or data.topic_name
    logger.info(f"User {current_user.email} requesting MCQ session for module/topic: {display_title}")
    sb = get_supabase_client()
    uid = current_user.supabase_uid
    ...
    try:
        ...
        log_backend_ai_usage(sb, uid, f"Practice: {display_title} (Cost: {credit_cost} Credits)", usage, source="backend")
